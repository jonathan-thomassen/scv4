...
TEST(test_move_right_solid) {
  ...
  int dy = 0;
  resolve_collision(T_WORLD_X, T_SPRITE_X, T_SPRITE_Y, &dx, &dy);
  /* dx should be clamped: right probe at pixel 55, query at 59, offset 3 → dx -= 3 = 1 */
  ASSERT_EQ(dx, 1);
  ASSERT_EQ(dy, 0);
}
...
TEST(test_move_up_right_wall_right) {
  ...
  int dy = -4;
  resolve_collision(T_WORLD_X, T_SPRITE_X, T_SPRITE_Y, &dx, &dy);
  /* Right wall hit: dx should be clamped to 1 */
  ASSERT_EQ(dx, 1);
 ...
 * off(1,2) UP-LEFT dx=-8 dy=-7: h: (34,65) T(4,8) off(2,1)  v: (42,58) T(5,7)
 * off(2,2)  hv: (34,58) T(4,7) off(2,2) DOWN-RIGHT dx=8 dy=7: h: (65,111)
...
TEST(test_middle_left_h_solid) {
  mock_grid_clear();
  /* Position B, dx=-8.  Left middle i=1: h at (34,80) T(4,10). */
...
TEST(test_ur_secondary_up_left_hv_solid) {
  mock_grid_clear();
  /* move_up_right_probe_up_left: hv at (left_x+dx, top_y+dy) = (50,58)
   * T(6,7) */
...
TEST(test_ur_secondary_up_left_v_solid) {
  mock_grid_clear();
  /* v at (left_x, top_y+dy) = (42,58) T(5,7). hv empty. */
...
TEST(test_ur_secondary_down_right_hv_solid) {
   ...
   * hv at (right_x+dx, bottom_y+dy) = (65, 104) T(8,13) */
  mock_grid_set(8, 13, true);
  int dx = 8;
  int dy = -7;
...
TEST(test_ur_secondary_down_right_h_solid) {
  mock_grid_clear();
  /* h at (right_x+dx, bottom_y) = (65,111) T(8,13). hv empty (use dy=-8). */
  mock_grid_set(8, 13, true);
  int dx = 8;
  int dy = -8;
  move_up_right_probe_down_right(B_WX, B_SX, B_SY, &dx, &dy);
...
TEST(test_ul_secondary_up_right_hv_solid) {
  mock_grid_clear();
  /* move_up_left_probe_up_right: hv at (right_x+dx, top_y+dy)
...
TEST(test_ul_secondary_up_right_v_solid) {
  mock_grid_clear();
  /* v at (right_x, top_y+dy) = (57,58) T(7,7). hv empty. */
...
TEST(test_ul_secondary_down_left_h_solid) {
  mock_grid_clear();
  /* h at (left_x+dx, bottom_y) = (34,111) T(4,13). hv empty (use dy=-8). */
  mock_grid_set(4, 13, true);
  int dx = -8;
  int dy = -8;
  move_up_left_probe_down_left(B_WX, B_SX, B_SY, &dx, &dy);
...
TEST(test_dr_secondary_down_left_hv_solid) {
   ...
   * left_x=42, dx=8, bottom=111, dy=7: (50,118) T(6,14) */
...
TEST(test_dr_secondary_down_left_v_solid) {
  mock_grid_clear();
  /* v at (left_x, bottom_y+dy) = (42,118) T(5,14). hv empty. */
  mock_grid_set(5, 14, true);
  int dx = 8;
  int dy = 7;
  move_down_right_probe_down_left(B_WX, B_SX, B_SY, &dx, &dy);
...
TEST(test_dr_secondary_up_right_hv_solid) {
  mock_grid_clear();
  /* move_down_right_probe_up_right: hv at (right_x+dx, top_y+dy)
   * right_x=57, dx=8, top_y=65, dy=7: (65,72) T(8,9) */
  mock_grid_set(8, 9, true);
  int dx = 8;
  int dy = 7;
...
TEST(test_dr_secondary_up_right_h_solid) {
  mock_grid_clear();
  /* h at (right_x+dx, top_y) = (65,65) T(8,8). hv empty. */
...
TEST(test_dl_secondary_down_right_hv_solid) {
   ...
   * right_x=57, dx=-8, bottom=111, dy=7: (49,118) T(6,14) */
  mock_grid_set(6, 14, true);
  int dx = -8;
  int dy = 7;
...
TEST(test_dl_secondary_down_right_v_solid) {
  mock_grid_clear();
  /* v at (right_x, bottom_y+dy) = (57,118) T(7,14). hv empty. */
  mock_grid_set(7, 14, true);
  int dx = -8;
  int dy = 7;
  move_down_left_probe_down_right(B_WX, B_SX, B_SY, &dx, &dy);
...
TEST(test_dl_secondary_up_left_hv_solid) {
  mock_grid_clear();
  /* move_down_left_probe_up_left: hv at (left_x+dx, top_y+dy)
   * left_x=42, dx=-8, top_y=65, dy=7: (34,72) T(4,9) */
  mock_grid_set(4, 9, true);
  int dx = -8;
  int dy = 7;
...
TEST(test_dl_secondary_up_left_h_solid) {
  mock_grid_clear();
  /* h at (left_x+dx, top_y) = (34,65) T(4,8). hv empty. */
...
TEST(test_dispatch_dy_positive) {
  /* down-left with left wall → dx zeroed, dy>0 → move_down_probes */
  mock_grid_clear();
...
TEST(test_dispatch_both_zeroed) {
   ...
   * h at (36,64) T(4,8) off(4,0): dx += 8-4 = 0.
   * v at (40,60) T(5,7) off(0,4): dy += 8-4 = 0.
